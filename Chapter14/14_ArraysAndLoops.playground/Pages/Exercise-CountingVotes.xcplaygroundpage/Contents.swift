...
if yesVotes.count > noVotes.count {
    print("The vote passed!")
} else {
    print("The vote failed. Better luck next time.")
}
...
func tallyVotes(forIssue: String, withVotes: [Bool]) -> String {
    ...
    if yesVotes.count > noVotes.count {
        return ("\(forIssue) \(yesVotes.count) yes, \(noVotes.count) no.")
    } else {
        return ("The vote failed. Better luck next time. \(yesVotes.count) yes, \(noVotes.count) no.")
    }
}

print(tallyVotes(forIssue: "Should we get a new mascot?", withVotes: shouldMascotChangeVotes))
